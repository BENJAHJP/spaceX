...
class GetAllStarLinksUseCase @Inject constructor(
    private val repository: SpacexRepository
){
    operator fun invoke(): Flow<Resource<List<StarLinkModel>>> = flow {
        try {
            emit(Resource.Loading())
            val starLinks = repository.getAllStarLinks().map { it.toStarLinkModel() }
            emit(Resource.Success(starLinks))
