...
class GetAllLaunchPadsUseCase @Inject constructor(
    ...
    operator fun invoke(): Flow<Resource<List<LaunchPadModel>>> = flow {
        try {
            emit(Resource.Loading())
            val launchPads = repository.getAllLaunchPads().map { it.toLaunchModel() }
            emit(Resource.Success(launchPads))
